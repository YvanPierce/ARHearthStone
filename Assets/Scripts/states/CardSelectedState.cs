...
namespace Assets.Scripts.states
{
    public class CardSelectedState : IAttackState
    {
        public void handleState(God god, string input = "")
        {
            ...
            {
                if (god.currentplayer.SelectedCard.Value.getCardType() == 0)  // 英雄牌，转到卡牌施放状态
                {
                ...
                if (hit)
                {
                    ...
                    if (god.currentplayer.hitobject.tag == "Card")      // 选中卡片
                    {
                        ...
                        else      // 点击不同卡片， 不切换状态
                        {
                            ...
                            Debug.Log("当前选中卡牌为: " + god.currentplayer.SelectedCard.Value.GetName() + " 耗费为" + god.currentplayer.SelectedCard.Value.GetCost());
                        }
